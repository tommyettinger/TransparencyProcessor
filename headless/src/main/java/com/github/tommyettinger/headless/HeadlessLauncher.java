...
@CommandLine.Command(name = "TransparencyProcessor", version = "TransparencyProcessor 0.0.1-SNAPSHOT",
...
public class HeadlessLauncher implements Callable<Integer> {
	...
	public Integer call() {
		...
		new HeadlessApplication(new TransparencyProcessor(input), configuration) {
			{
				try {
					mainLoopThread.join();
